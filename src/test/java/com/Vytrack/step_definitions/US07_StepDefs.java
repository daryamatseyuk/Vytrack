...
public class US07_StepDefs {
    ...
    final int randomNumber = random.nextInt(24)+1;
    ...
    public void user_should_see_that_all_checkboxes_are_selected() {
        System.out.println(vehiclesModelPage.selectedElementsList.size());
        //for (int i=0; i<10;i++){
            //Assert.assertTrue(vehiclesModelPage.allCheckboxes.get(i).isSelected());
        //}
        vehiclesModelPage.selectedElementsList.forEach(x->Assert.assertTrue(x.isSelected()));
    }
    ...
    public void user_clicks_on_any_random_checkbox_to_select_a_vehicle() {
        target = vehiclesModelPage.allCheckboxes.get(randomNumber);
        target.click();
    }
